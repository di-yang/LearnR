---
title: "Result with weight; control for racial disparity; droped slope for highschool, PCP, unemployment; slope as dependent variable"
output:
  html_document:
    df_print: paged
---
...
analyticnweight=merge(analyticb, weight,  by.x="FIPS", by.y="FIPS")
...
analyticwrace=merge(analyticnweight, tomerge, by.x="FIPS", by.y="FIPS")
...
library(pdp)
set.seed (1)
YPLLanalytic=analyticwrace[-c(2,3,4,21)]
...
inTraining = createDataPartition(YPLLanalytic$YPLLb, p = .75, list = FALSE)
...
xgbFit1 = train(YPLLb ~ ., data = training,
                 method = "xgbTree", 
...
predvarls = list("somecollegea", "inactivitya", "obesitya", "inactivityb",  "unemploymenta", "uninsureda", "highschoola", "unemploymentb", "blwt", "PCPRateb")
...
res.partialplot[[m]] = plotPartial(xgb.pdp[[m]], rug =TRUE, train = training, ylim=c(30,140) )
}
...
library(pdp)
set.seed (1)
mentalanalytic=analyticwrace[-c(1,2,3,21)]
nc = parallel::detectCores()  
cl = makePSOCKcluster(nc-1)   # Set number of cores equal to machine number minus one
registerDoParallel(cl)        #Set up parallel

inTraining = createDataPartition(mentalanalytic$mentalb, p = .75, list = FALSE)
...
xgbFit2 = train(mentalb ~ ., data = training,
                 method = "xgbTree", 
                 trControl = fitControl,
                 tuneGrid = xgbGrid2,
                weights = analyticwrace$averweight[inTraining],
...
predvarls2 = list("unemploymenta", "unemploymentb", "inactivitya", "obesitya", "highschoola", "hpnon", "uninsuredb", "childpovertya", "highschoolb", "obesityb")
...
res.partialplot2[[m]] = plotPartial(xgb.pdp2[[m]], rug =TRUE, train = training, ylim=c(0.02, 0.16))
}
...
library(pdp)
set.seed (1)
physicalanalytic=analyticwrace[-c(1,2,4,21)]

nc = parallel::detectCores()  
cl = makePSOCKcluster(nc-1)   # Set number of cores equal to machine number minus one
registerDoParallel(cl)        #Set up parallel

inTraining = createDataPartition(physicalanalytic$physicalb, p = .75, list = FALSE)
...
xgbFit3 = train(physicalb ~ ., data = training,
                 method = "xgbTree", 
                 trControl = fitControl,
                 tuneGrid = xgbGrid3,
                weights = analyticwrace$averweight[inTraining],
...
predvarls3 = list("somecollegea", "inactivitya", "blwt", "obesityb", "childpovertya", "highschoolb", "uninsureda", "childpovertyb", "highschoola", "somecollegeb")
...
res.partialplot3[[m]] = plotPartial(xgb.pdp3[[m]], rug =TRUE, train = training, ylim=c(0, 0.08))
}
...
library(pdp)
set.seed (1)
fairpooranalytic=analyticwrace[-c(1,3,4,21)]
nc = parallel::detectCores()  
cl = makePSOCKcluster(nc-1)   # Set number of cores equal to machine number minus one
registerDoParallel(cl)        #Set up parallel

inTraining = createDataPartition(fairpooranalytic$fairpoorb, p = .75, list = FALSE)
...
xgbFit4 = train(fairpoorb ~ ., data = training,
                 method = "xgbTree", 
                 trControl = fitControl,
                 tuneGrid = xgbGrid4,
                weights = analyticwrace$averweight[inTraining],
...
predvarls4 = list("somecollegea", "inactivitya", "childpovertyb", "unemploymentb", "uninsureda", "highschoolb", "childpovertya", "unemploymenta", "inactivityb", "obesitya")
...
res.partialplot4[[m]] = plotPartial(xgb.pdp4[[m]], rug =TRUE, train = training, ylim=c(-0.4, 0.6) )
}
